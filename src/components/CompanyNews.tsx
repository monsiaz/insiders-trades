...
export function CompanyNews({ slug, companyName }: { slug: string; companyName: string }) {
  ...
  if (!data || data.items.length === 0) {
    // If no ticker at all (no-symbol), hide the whole section — nothing to say
    if (data?.reason === "no-symbol") return null;
    return (
      <div
        style={{
      ...
      >
        ...
        >
          Aucune dépêche récente via Yahoo Finance pour cette société.
        </p>
      </div>
    );
  }
